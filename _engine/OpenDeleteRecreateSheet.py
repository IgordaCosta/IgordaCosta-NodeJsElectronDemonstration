...
def OpenDeleteRecreateSheet():
    ...
    # print('columnList above')

    frame=pandas.DataFrame(columns=columnList)
    ...
    if ProblemSaving:
        ProblemSavingTrue(filename,frame,columnList, ListToPrint)

    else:
        # print("Done this part, go to next")
        SavedOK(filename,frame,columnList, ListToPrint)
                
        

def ProblemSavingTrue(filename,frame,columnList, ListToPrint):
    ...
    if SavedFile:
        ...
        ListToPrint.append("did NOT save")

        PrintTexListSerial.PrintTexListSerial(ListToPrint=ListToPrint)

    
                    
def SavedOK(filename,frame,columnList, ListToPrint):
    ...
    try:
        ...
        xl.Workbooks(filename).Close(True)
        # print("step file closed" )
        writer = pandas.ExcelWriter(filename)
