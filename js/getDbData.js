...
function getDbData (resultDatabase,TableName,Database,CurrentWorkingPath,MydocumentsDbPath) {
  ...
  // console.log('get db data this should appear mid')
...
async function getDbData0 (resultDatabase,TableName,Database) {
  ...
  let names000 = await results[results.length - 1];

  let Datalist000 = await results[results.length - 2];
...
// const result = '5+2+1'.replace(searchRegExp1, replaceWith);
  ...
  // console.log('result of getDbdata function');

  // console.log('this is the before return of get db data (this should appear first)')
